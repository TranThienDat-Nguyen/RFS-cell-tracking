function AOGMR(aResPath, aGtPath, threshold, aCosts)
...
wbar = waitbar(0, '', 'Name', 'Computing TRA score');
...
%     end
    ...
    parfor res = 2 : resNum % dont count the label pixel
        curr_res_lab = resLabels(res) ;  
        [x_res, y_res] = find(resIm == curr_res_lab) ; 
        if length(x_res)>1
            x_res = mean(x_res) ; 
            y_res = mean(y_res) ;
        end
        for gt = 2 : gtNum
            curr_gt_lab = gtLabels(gt) ; 
            [x_gt, y_gt] = find(gtIm == curr_gt_lab) ;  
            overlaps(gt,res) = sqrt( (x_res-x_gt)^2 + (y_res-y_gt)^2 ) ; 
        end
    end
...
%     gtCounts = gtCounts(2:end);
    ...
    if isempty(overlaps)
        ...
        [num_rows, num_cols] = size(overlaps) ; 
        matches = zeros(size(overlaps)) ; 
        if  num_rows>num_cols
            [matches_min, ~] = lapjv(overlaps') ; % matches_min: column solution
            for c = 1 : length(matches_min)
                matches(matches_min(c),c) = 1 ; 
            end
        else
            [matches_min, ~] = lapjv(overlaps) ;  % matches_min: row solution
            for r = 1 : length(matches_min)
                matches(r, matches_min(r)) = 1 ; 
            end
        end
        actual_matches = inf(size(matches)) ; 
        actual_matches(matches>0) = overlaps(matches>0) ; 
        matches = actual_matches<threshold ; 
    end
    
    % Insert matching region indices into gtMatches and resMatches.
    % Computed regions which are assigned to more than one ground truth are
    ...
    for i = 1:gtNum
        ...
        if ~isempty(match) && resMatchSum(match) == 1
            r = resLabels(match);
            g = gtLabels(i);
            if (t-gtTracks.start(g)) ~= 0 
                gtMatches{g}(t-gtTracks.start(g)) = r;
            else
                gtMatches{g}(1) = r;
            end
            
            if (t-resTracks.start(r)) ~= 0
