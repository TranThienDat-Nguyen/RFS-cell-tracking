function [w_pred , m_out , P_out] = kalman_predict_multi_object(model , w_kin_in , m_kin_in , P_kin_in , spawn_idx , marg_flag)
...
function [ w_kin_pred , m_kin_pred , P_kin_pred ] = kalman_predict(model,w_trans,F,Q, w_kin_in , m_kin_in , P_kin_in , spawn_idx)
    ...
    for cidx = 1 : length(w_kin_in)
        ...
        if ~isempty(spawn_idx) % if yes, stacking matrices on top of each other
            d = zeros (numObjout*model.x_dim , length(model.wsp)) ; % shifting vector for spawning tracks 
            for sgidx = 1 : length(model.wsp)
                d(model.xy_pos,sgidx) = [model.x_shift(sgidx) ; model.y_shift(sgidx)] ; 
                d(model.xy_pos+model.x_dim,sgidx) = [-model.x_shift(sgidx) ; -model.y_shift(sgidx)] ; 
                w_sets = model.wsp ; 
            end 
        else
